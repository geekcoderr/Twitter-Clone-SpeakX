...
const ProfilePage = () => {
	...
	const [postLength, setPostLength] = useState(
		JSON.parse(localStorage.getItem("userTweets")) || 0
	);
		...
		if (user) {
			const fetchPosts = async () => {
				...
				if (res.ok) {
					...
					localStorage.setItem("userTweets", JSON.stringify(data.length));
				}
			};
			fetchPosts();
		}
					...
					{!isLoading && !isRefetching && user && (
						<>
							...
							<div className='flex w-full border-b border-gray-700 mt-4'>
								<div
									className='flex justify-center flex-1 p-3 hover:bg-secondary transition cursor-pointer'
							...
							<Posts
								...
								userId={user?._id}
								handlePostCountUpdate={setPostLength}
								refetch={refetch}
							/>
						</>
					)}
